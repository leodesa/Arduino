...
void loop() {
  ultrasom = sonar.ping_cm();
  contFrente = digitalRead(sensorFrente);
  contTras = digitalRead(sensorTras);
  if (contFrente == 1 and contTras == 0) {
    digitalWrite(R1T, HIGH);
    digitalWrite(R2T, HIGH);
    digitalWrite(R3T, HIGH);
    digitalWrite(R4T, HIGH);
  ...
  if (contTras == 1 and contFrente == 0) {
    digitalWrite(R1F, HIGH);
    digitalWrite(R2F, HIGH);
    digitalWrite(R3F, HIGH);
    digitalWrite(R4F, HIGH);
  ...
  if (contFrente == 1 and contTras == 1) {
    parar();
  }
  if (ultrasom == 0) {
    parar();
  ...
  if (contFrente == 0 and contTras == 0 and ultrasom == 0) {
    irEsquerda();
  } else {}
}
